...
def axis_to_internal(axis_label: str) -> str:
    mapping = {
        "x": "x",
        "y": "y",
        "z": "z",
        "Vsum(x,y,z)": "vsum(x,y,z)",
        "max(x,y,z)": "max(x,y,z)",
    }
    return mapping[axis_label]


def choose_freq_ticks(fig_height_px: int):
    target_labels = max(4, int(fig_height_px / 120))
    n = len(THIRD_OCTAVE_BANDS)
    step = max(1, math.ceil(n / target_labels))
    ticks = THIRD_OCTAVE_BANDS[::step]
    if THIRD_OCTAVE_BANDS[-1] not in ticks:
        ticks = np.append(ticks, THIRD_OCTAVE_BANDS[-1])
    return ticks
...
def add_plot_trace(fig, row, times, freqs, matrix_mm_s, low_label, high_label, banded, location, axis_label, colorbar_x):
...
def apply_dynamic_freq_ticks_to_fig(fig, plot_count):
    height_per_plot = max(220, int(fig.layout.height / max(plot_count, 1)))
    ticks = choose_freq_ticks(height_per_plot)
    ticktext = [format_decimal(v) for v in ticks]
    for row in range(1, plot_count + 1):
